...
def list_variables(checkpoint_dir,shape=False):
...
def main():
    checkpoints = tf.train.latest_checkpoint("records/all")
    tr_checkpoints = tf.train.latest_checkpoint("records/trainable")
    variables_ch= list_variables(checkpoints)
    variables_tr=list_variables(tr_checkpoints)
    print (len(variables_ch))
    print(variables_ch)
    print(len(variables_tr))
    print(variables_tr)

    initial = tf.constant(0.1, shape=[32])
    check_biases = tf.get_variable(name='cnn/conv1_biases', initializer=initial)
    restore_val={}

    name = 'cnn/conv1_biases'
    restore_val[name]=check_biases
    saver = tf.train.Saver(restore_val)

    with tf.Session() as sess:
        sess.run(tf.variables_initializer([check_biases]))
        saver.restore(sess,checkpoints)
        print(check_biases.eval())



    # varible_s = [v for v in variables_ch if not v[0].startswith('cnn/cnn/fc3/fc3_')]
    # print(len(varible_s))
    # print(varible_s)
    # print_tensors_in_checkpoint_file(file_name=checkpoints, tensor_name='cnn/fc1/fc1_biases',all_tensors=False)
    # print_tensors_in_checkpoint_file(file_name=checkpoints, tensor_name='cnn/fc1/fc1_biases',
    #                                 all_tensors=False)
